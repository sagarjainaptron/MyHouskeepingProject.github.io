 ...
 */
@MultipartConfig
public class DeleteStaffAttendant extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            int id=Integer.parseInt( request.getParameter("del_st_id"));
           Staff_dao dao=new Staff_dao(ConnectionProvider.getConnection());
           Staff_update sa=new Staff_update(id);
           boolean ans=dao.deleteStaffAttendant(sa);
            if(ans){
                out.println("deleted");
            }
            else{
